...
DISABLE_LLM = os.getenv("DISABLE_LLM", "0").strip() == "1"
...
def _call_with_timeout(fn, timeout_seconds: float, label: str):
    """
    Run a potentially-blocking API call with a hard timeout.

    Note: Threads cannot be force-killed in Python, but timing out here prevents the
    request path from hanging indefinitely and allows fallbacks to kick in.
    """
...
"""
    ...
    # If Groq isn't configured (or LLM is disabled), keep the system usable with a deterministic fallback.
    if DISABLE_LLM or not GROQ_API_KEY:
        reason = "DISABLE_LLM=1" if DISABLE_LLM else "missing GROQ_API_KEY"
        analysis = _fallback_analysis(safe_company, safe_metrics, reason=reason)
    else:
        try:
            ...
            timeout_s = _env_float("GROQ_SNAPSHOT_TIMEOUT_SECONDS", 35)
            response = _call_with_timeout(_do_call, timeout_seconds=timeout_s, label="Groq snapshot analysis")
...
def generate_scorecard_narrative(result: dict) -> str:
    if DISABLE_LLM:
        return "Narrative disabled (DISABLE_LLM=1)."
    prompt = build_narrative_prompt(result)
    client = get_llm_client()

    narrative = ""
    timeout_s = _env_float("GROQ_NARRATIVE_TIMEOUT_SECONDS", 20)
    for attempt in range(2):
        ...
        response = _call_with_timeout(_do_call, timeout_seconds=timeout_s, label="Groq scorecard narrative")
...
def generate_chart_intel_verdict(ticker: str, price_change: float, signals: dict) -> str:
    ...
    timeout_s = _env_float("GROQ_CHART_TIMEOUT_SECONDS", 12)
    response = _call_with_timeout(_do_call, timeout_seconds=timeout_s, label="Groq chart intelligence")
