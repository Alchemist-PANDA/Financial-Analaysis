...
const MainTerminal = ({ forceTicker, onAnalysisComplete, onDataLoaded, onTickerChange }: MainTerminalProps) => {
    ...
    const handleAnalyze = useCallback(async (targetTicker?: string, manualPayload?: ManualAnalysisInput) => {
        ...
        // Production performance mode: use the fast JSON endpoint (cache-first) instead of SSE.
        // This avoids long-running streams and guarantees we can hard-timeout quickly.
        const analysisUrl = `${BASE_URL}/api/analyze?ticker=${encodeURIComponent(resolvedTicker)}`;
        ...
        try {
            ...
            const normalized = normalizeResultPayload(response.data, resolvedTicker);
            if (!normalized) {
    ...
    }, [ticker, closeEventSource, runManualAnalysis, BASE_URL, API_KEY, finalizeAnalysis, onDataLoaded, onTickerChange, formatProgress]);
    ...
    const handleExportPDF = useCallback(async () => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                const fallbackMessage = 'Failed to export PDF report.';

                if (contentType.includes('application/json')) {
                    try {
                        const errorPayload = await response.json();
                        throw new Error(errorPayload?.detail || errorPayload?.error || fallbackMessage);
                    } catch {
                        throw new Error(fallbackMessage);
                    }
                }

                // If the backend returns HTML (HF sleeping/protection), avoid JSON parsing explosions.
                const text = await response.text();
                const lowered = text.toLowerCase();
                if (
                    lowered.includes('<html') &&
                    (lowered.includes('sign in') || lowered.includes('private space') || lowered.includes('request access'))
                ) {
                    throw new Error(
                        'Hugging Face Space is protected/private (login required). Make it Public or disable Protection/Gating and retry.'
                    );
                }
                if (lowered.includes('<html') || lowered.includes('hugging face')) {
                    throw new Error('Backend is waking up (or rebuilding). Wait ~20 seconds and retry.');
                }
                throw new Error(fallbackMessage);
            }
    ...
    const handleScorecardModeChange = useCallback(async (mode: 'credit' | 'investment') => {
        ...
        setScorecardMode(mode);
        try {
            const response = await safeFetch(`${BASE_URL}/api/scorecard/analyze`, {
                method: 'POST',
                headers: {
                    'X-API-Key': API_KEY,
                },
            ...
            setScorecardResult(response.data as ScorecardResult);
        } catch (err) {
